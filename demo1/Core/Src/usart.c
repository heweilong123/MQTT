...
extern uint8_t rx_buffer_wifi[200];
/* USER CODE END 0 */
...
{
  HAL_UART_Transmit(&hlpuart1, (uint8_t *)&ch, 1, 5);
  return ch;
}
...
{
  memcpy(tx_buffer_cmd, data, len);	
  BDMA->IFCR |= 0xF0;
  HAL_UART_Transmit_DMA(&hlpuart1, tx_buffer_cmd, len);
	//等待传输完成
