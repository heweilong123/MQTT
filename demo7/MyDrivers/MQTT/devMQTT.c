...
const char MQTT_CLIENTID[] =  "589493207";
const char MQTT_USERNAME[] =  "330283";
const char MQTT_USERPAWD[] =  "8266";
...
HAL_StatusTypeDef MQTT_PUBLISH(char* topic, uint8_t serviceLevel, const char* data)
{
    ...
    uint8_t number = 0;
    MQTT_SEND_BUFFER[number ++] = 0x30 | ((serviceLevel & 0x03)<<1);
    MQTT_SEND_BUFFER[number ++] = 0xFF;
    ...
    if(serviceLevel != 0x00){
        MQTT_SEND_BUFFER[number ++] = packetID / 256;
        MQTT_SEND_BUFFER[number ++] = packetID % 256;
    }

    for(uint8_t i = 0; i < strlen(data); i++)
        MQTT_SEND_BUFFER[number ++] = *(data + i);
    ...
    if(serviceLevel == 0x00)
        return HAL_OK;
    else if(serviceLevel == 0x01){
        char recData[] = {0x40, 0x02, packetID / 256, packetID % 256};
        if(MQTT_BACKDATA_VERIFY(recData) == HAL_OK){
            printf("MQTT PUBLISH QOS1 \"%s\" TRUE\r\n", topic);
            return HAL_OK;
        }
        else{
            printf("MQTT PUBLISH QOS1 \"%s\" ERROR\r\n",topic);
            return HAL_ERROR;
        }   
    }
    else if(serviceLevel == 0x02){
        char recData[] = {0x50, 0x02, packetID / 256, packetID % 256};
        if(MQTT_BACKDATA_VERIFY(recData) == HAL_OK){
            printf("MQTT PUBLISH QOS2 PUBREC \"%s\" TRUE\r\n", topic);
            if(MQTT_PUBREL(packetID) == HAL_OK){
                printf("MQTT PUBLISH QOS2 PUBREL \"%s\" TRUE\r\n", topic);
                return HAL_OK;
            }
            else{
                printf("MQTT PUBLISH QOS2 PUBREL \"%s\" ERROR\r\n",topic);
                return HAL_ERROR;
            }   
        }
        else{
            printf("MQTT PUBLISH QOS2 PUBREC \"%s\" ERROR\r\n",topic);
            return HAL_ERROR;
        }   
    }
	else
...
HAL_StatusTypeDef MQTT_PUBREL(uint16_t id)
{
    ...
    char recData[] = {0x70, 0x02, packetID / 256, packetID % 256};
    if(MQTT_BACKDATA_VERIFY(recData) == HAL_OK){
        printf("MQTT PUBLISH QOS2 PUBREL TRUE\r\n");
        return HAL_OK;
    }
    else{
        printf("MQTT PUBLISH QOS2 PUBREL ERROR\r\n");
        return HAL_ERROR;
    }   
}
    ...
    if(MQTT_RECFLAG == 1){
        ...
        if(MQTT_REC_BUFFER[0] == 0x30){
            ...
            if(strstr((const char*)recTopic, "test") != NULL){
                ...
                MQTT_PUBLISH("topic", 0x02, recData);
            }
